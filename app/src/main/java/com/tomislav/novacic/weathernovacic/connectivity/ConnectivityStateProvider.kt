...
interface ConnectivityStateProvider {
    ...
    sealed class NetworkState {
        ...
        sealed class ConnectedState(val hasNetworkConnection: Boolean) : NetworkState() {
            ...
            data class ConnectedLegacy(val networkInfo: NetworkInfo) :
                ConnectedState(networkInfo.isConnectedOrConnecting)
        }
    }

    companion object {
        fun createProvider(context: Context): ConnectivityStateProvider {
            val cm = context.getSystemService(CONNECTIVITY_SERVICE) as ConnectivityManager
            return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ConnectivityStateProviderImpl(cm)
            } else {
                ConnectivityStateProviderLegacyImpl(context, cm)
            }
        }
    }
}